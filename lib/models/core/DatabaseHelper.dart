...
class DatabaseHelper {
  ...
  void _createDb(Database db, int newVersion) async {
    await db.execute(
        'CREATE TABLE $contaTable($colId INTEGER PRIMARY KEY AUTOINCREMENT, $colNome TEXT, '
  ...
  Future<Contato> getContato(int id) async {
    ...
    List<Map> maps = await db.query(contaTable,
        columns: [colId, colNome, colImage],
        where: "$colId =?",
        whereArgs: [id]);
  ...
  Future<int> deleteContacto(int id) async {
    ...
    int resultado =
        await db.delete(contaTable, where: "$colId = ?", whereArgs: [id]);
    return resultado;
  }
}
